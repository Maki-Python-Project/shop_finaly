...
def product_spec(category):
    ...
    for product_feature in product_features:
        if product_feature.feature.unit == None:
            product_feature.feature.unit = ' '
        if product_feature.value not in feature_and_values[(product_feature.feature.feature_name, product_feature.feature.unit)]:
            feature_and_values[
                (product_feature.feature.feature_name, product_feature.feature.unit)
    ...
    for (feature_name, feature_filter_name), feature_values in feature_and_values.items():
        ...
        for f_v in feature_values:
            mid_feature_values_res = \
                "<input type='checkbox' name='{f_f_name}' value='{feature_name}'> {feature_name} {f_f_name}</br>".format(
